...
// FIX: previously these strings were hardcoded Roman Urdu (e.g. "RSI 30 se
// neeche = oversold...", "PRICE UP JAYE GA") regardless of site locale. Now
// every locale supported by the site (see VALID_LOCALES in page.jsx) gets a
// proper, natural translation, following the same per-locale dictionary
// pattern used in ABOUT_DICT.
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function CoinAnalysisPage({ initialData = null, locale = "en" }) {
  ...
  // FIX: track whether we've already consumed the server-provided initialData.
  // Previously `initialData` itself was checked inside the effect, but it never
  // changes after mount — so once it was truthy (Bitcoin's SSR data), clicking
  // any other coin button updated `coinId` but the effect's `if (!initialData)`
  // guard stayed false forever, and analyze() never ran again for new coins.
  ...
  useEffect(() => {
    // First run: if we already have SSR data for this exact coin, just mark it
    // consumed and skip the fetch (avoids a redundant request on initial load).
    if (!usedInitialData.current && initialData) {
      usedInitialData.current = true;
      return;
    }
    // FIX: every subsequent coinId change (including switching back to the
    // initial coin) now correctly triggers a fresh fetch.
    analyze(coinId);
...
function GaugeBar({ label, value, min = 0, max = 100, low = 30, high = 70 }) {
  if (value === null) return <div className="coin-app-gauge-wrap"><div className="coin-app-gauge-label">{label}</div><div className="coin-app-gauge-na">N/A</div></div>;
...
function HistoryTab({ data }) {
  ...
  if (!ohlc7d || ohlc7d.length === 0) return <div className="coin-app-tab-content"><p className="coin-app-no-data">Historical data not available</p></div>;
            ...
            return (
              <div key={i} className="coin-app-candle-col">
                <div className="coin-app-candle-wrap" title={`O:${d.open} H:${d.high} L:${d.low} C:${d.close}`}>
                  ...
                  {/* Body */}
                  <div
                    className="coin-app-candle-body"
                    style={{
...
// ─── CSS ──────────────────────────────────────────────────────────────────────
// THEME: This project uses Tailwind's class-based dark mode — dark styles
// activate when an ancestor (typically <html>) has class="dark", added by the
// site's theme toggle in the layout/navbar. This component previously used
// hardcoded hex colors in plain CSS, which Tailwind's .dark class has no way
// to affect (two separate systems). FIX: every color is now a CSS variable
// declared on `.app` (light values) and overridden inside `.dark .app` (dark
// values) — so this same plain-CSS block now responds to the site's existing
// dark-mode toggle automatically. Layout, spacing, class names, and structure
// are 100% unchanged — only color values were converted to var(--token) refs.
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Theme tokens: DARK (when an ancestor has class="dark") ─────────────── */
  .dark .coin-app-app {
    --bg: #050a14;
    --text: #e8f0fe;
    --text-soft: #8899aa;
  ...
  .coin-app-quick-btn.coin-app-active { background: color-mix(in srgb, var(--accent) 12%, transparent); border-color: var(--accent); color: var(--accent-strong); }
  ...
  .coin-app-internal-links { display: flex; flex-wrap: wrap; gap: 0.5rem; }
  .coin-app-internal-links a { font-size: 0.8rem; color: var(--accent-strong); text-decoration: none; padding: 0.2rem 0.6rem; border: 1px solid color-mix(in srgb, var(--accent) 33%, transparent); border-radius: 4px; transition: all 0.2s; }
  .coin-app-internal-links a:hover { background: color-mix(in srgb, var(--accent) 12%, transparent); border-color: var(--accent); }
  ...
  .coin-app-change-badge.coin-app-pos { background: color-mix(in srgb, var(--green-strong) 15%, transparent); color: var(--green); border: 1px solid color-mix(in srgb, var(--green-strong) 40%, transparent); }
  .coin-app-change-badge.coin-app-neg { background: color-mix(in srgb, var(--red-strong) 15%, transparent); color: var(--red); border: 1px solid color-mix(in srgb, var(--red-strong) 40%, transparent); }
  .coin-app-hero-right { min-width: 160px; }
  .coin-app-signal-box { text-align: center; padding: 1rem 1.5rem; border-radius: 12px; }
  .coin-app-signal-box.coin-app-bull { background: color-mix(in srgb, var(--green-strong) 12%, transparent); border: 1px solid color-mix(in srgb, var(--green-strong) 40%, transparent); }
  .coin-app-signal-box.coin-app-bear { background: color-mix(in srgb, var(--red-strong) 12%, transparent); border: 1px solid color-mix(in srgb, var(--red-strong) 40%, transparent); }
  .coin-app-signal-box.coin-app-neutral { background: color-mix(in srgb, var(--orange-strong) 12%, transparent); border: 1px solid color-mix(in srgb, var(--orange-strong) 40%, transparent); }
  ...
  .coin-app-tab-btn.coin-app-active { background: color-mix(in srgb, var(--accent) 12%, transparent); border-color: var(--accent); color: var(--accent-strong); font-weight: 600; }
  ...
  .coin-app-big-signal.coin-app-bull { background: radial-gradient(circle at center, color-mix(in srgb, var(--green-strong) 14%, transparent), var(--surface-soft)); border: 1px solid color-mix(in srgb, var(--green-strong) 44%, transparent); }
  .coin-app-big-signal.coin-app-bear { background: radial-gradient(circle at center, color-mix(in srgb, var(--red-strong) 14%, transparent), var(--surface-soft)); border: 1px solid color-mix(in srgb, var(--red-strong) 44%, transparent); }
  .coin-app-big-signal.coin-app-neutral { background: radial-gradient(circle at center, color-mix(in srgb, var(--orange-strong) 14%, transparent), var(--surface-soft)); border: 1px solid color-mix(in srgb, var(--orange-strong) 44%, transparent); }
  ...
  .coin-app-reason-neutral { background: color-mix(in srgb, var(--orange-strong) 10%, transparent); border: 1px solid color-mix(in srgb, var(--orange-strong) 30%, transparent); }
  ...
  @media (max-width: 768px) {
    ...
    .coin-app-internal-links { gap: 0.4rem; }
  }
`